...
<div class="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-blue-50">
    <div class="relative overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-6 py-8">
            <!-- Navigation -->
            <div class="mb-8">
                <a href="courses.php" class="inline-flex items-center text-[#1E3A8A] hover:text-[#BFA14A] font-medium transition-all duration-200 group mt-10">
            ...
            <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                <?php foreach ($course['years'] as $year): ?>
                    <?php
                        ...
                        $firstSemester = $year['semesters'][0]['semester_number'] ?? '1';
                        $lastSemester = end($year['semesters'])['semester_number'] ?? $firstSemester;
                    ?>
                    <div class="group relative">
                        ...
                        <div class="relative bg-white rounded-3xl shadow-xl border border-gray-100 p-8 text-center transform group-hover:-translate-y-2 transition-all duration-300">
                            <div class="relative mb-6">
                                <div class="w-20 h-20 bg-[#1E3A8A] rounded-2xl flex items-center justify-center mx-auto shadow-lg group-hover:shadow-xl transition-shadow duration-300">
                                    <span class="text-3xl font-bold text-white"><?= htmlspecialchars($year['year_number']) ?></span>
                                </div>
                                <div class="absolute -top-2 -right-2 w-8 h-8 bg-[#1E3A8A] rounded-full flex items-center justify-center">
                            ...
                            <div class="flex items-center justify-center gap-8 mb-8">
                                <div class="text-center">
                                    <div class="text-3xl font-bold text-[#1E3A8A] mb-1"><?= $totalSemesters ?></div>
                                    <div class="text-sm text-[#1E3A8A] font-medium">Semesters</div>
                                </div>
                                <div class="w-px h-12 bg-[#BFA14A]"></div>
                                <div class="text-center">
                                    <div class="text-3xl font-bold text-[#1E3A8A] mb-1"><?= $totalSubjects ?></div>
                                    <div class="text-sm text-[#1E3A8A] font-medium">Total Subjects</div>
                                </div>
                            </div>
                            <div class="mb-8">
                                <span class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-gray-50 to-gray-100 rounded-full text-gray-600 text-sm font-medium">
                                    ...
                                    Semester <?= $firstSemester ?> - <?= $lastSemester ?>
                                </span>
                            </div>
                            <a href="year_semesters.php?course_id=<?= urlencode($course['id']) ?>&year=<?= urlencode($year['year_number']) ?>"
                               class="inline-flex items-center justify-center w-full px-8 py-4 bg-[#1E3A8A] text-white font-semibold rounded-xl hover:bg-[#BFA14A] shadow-lg hover:shadow-xl">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
